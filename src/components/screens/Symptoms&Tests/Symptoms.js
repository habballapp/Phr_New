...
const actions = [
    {
        text: "Add Symptoms",
        icon: require("../../../assets/symptoms.png"),
        name: "AddSymptoms",
        position: 1,
        color: '#48AE2D'
...
export default class Symptoms extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            list: [
                {
                    id: 1,
                    title: 'View Symptoms & Tests',
                    tableHead: ['Symptoms Type', 'Date', 'Time', 'Description', 'Test Date', 'Test Result'],
                    widthArr: [40, 60, 80, 100, 120, 140, 160, 180, 200],
                    tableData: [
                    ],
    ...
    takeData() {
        ...
        var refr = firebase.database().ref('Register_User/' + userUID + '/AddSymptoms&Tests/')
        ...
        refr.on("value", (snapshot) => {
            ...
            snapshot.forEach((data) => {
                ...
                this.setState({ url: data.val().TestResult })
                const bio_data = [


                    data.val().SymptomsType,
                    data.val().Date,
                    data.val().Time,
...
const styles_table = StyleSheet.create({
    ...
    dataWrapper: { marginTop: -1 },
    row: { height: 180, backgroundColor: '#E7E6E1' }
});
